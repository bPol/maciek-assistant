const isTodoProvider = (value) => {
    if (!value || typeof value !== "object") {
        return false;
    }
    return typeof value.listTasks === "function";
};
...
const summarizeTasks = (tasks, now) => {
...
const proposeHelp = (tasks, now) => {
    ...
    if (overdue.length > 0) {
        suggestions.push(`Triage ${overdue.length} overdue task${overdue.length === 1 ? "" : "s"} and reset due dates.`);
    }
    const noDueDate = tasks.filter((task) => !task.dueAt);
    if (noDueDate.length > 0) {
        suggestions.push(`Add due dates to ${noDueDate.length} task${noDueDate.length === 1 ? "" : "s"} to reduce drift.`);
    }
    const highPriority = tasks.filter((task) => task.priority && /high|urgent|p1/i.test(task.priority));
    if (highPriority.length > 0) {
        suggestions.push(`Focus on ${highPriority.length} high-priority task${highPriority.length === 1 ? "" : "s"} today.`);
    }
